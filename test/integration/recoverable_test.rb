...
class PasswordTest < ActionController::IntegrationTest
  ...
  test 'change password with valid parameters in XML format should return valid response' do
    user = create_user
    request_forgot_password
    put user_password_path(:format => 'xml'), :user => {:reset_password_token => user.reload.reset_password_token, :login_password => '987654321', :login_password_confirmation => '987654321'}
    assert_response :success
    assert warden.authenticated?(:user)
  end
  ...
  test 'change password with invalid token in XML format should return invalid response' do
    user = create_user
    request_forgot_password
    put user_password_path(:format => 'xml'), :user => {:reset_password_token => 'invalid.token', :login_password => '987654321', :login_password_confirmation => '987654321'}
    assert_response :unprocessable_entity
    assert response.body.include? %(<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<errors>)
  end
  ...
  test 'change password with invalid new password in XML format should return invalid response' do
    user = create_user
    request_forgot_password
    put user_password_path(:format => 'xml'), :user => {:reset_password_token => user.reload.reset_password_token, :login_password => '', :login_password_confirmation => '987654321'}
    assert_response :unprocessable_entity
    assert response.body.include? %(<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<errors>)
  end
